      ...
      <div class="col">
        ...
        <form-tag v-if="this.ready" @userEditEvent="submitHandler" name="userform" event="userEditEvent">
          ...
          <text-input v-if="this.user.id === 0" v-model="user.password" type="password" required="true" label="Password"
            :value="user.password" name="password"></text-input>
          ...
          <div>
            ...
            <router-link to="/admin/users" class="btn btn-outline-secondary mr-2">Cancel</router-link>

            <a v-if="(this.$route.params.userId > 0) && (parseInt(String(this.$route.params.userId), 10) !== store.user.id)"
              class="btn btn-outline-danger" href="javascript:void(0);" @click="confirmDelete(this.user.id)">Delete</a>
          </div>
