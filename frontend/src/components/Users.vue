      ...
      <table v-if="this.ready" class="table table-compact table-striped">
        ...
        <tbody>
          <tr v-for="u in this.users" v-bind:key="u.id">
            ...
            <td v-if="u.active === 1">
              <span class="badge bg-info logged-in">Active</span>
            </td>
            <td v-else>
            ...
            <td v-if="u.token.id > 0">
              <a href="javascript:void(0);">
                <span class="badge bg-info logged-in" @click="logUserOut(u.id)">Logged in</span>
              </a>
            </td>
            <td v-else>
      ...
      if (id !== store.user.id) {
        notie.confirm({
          ...
          submitCallback: () => {
            ...
            fetch("http://localhost:8081/admin/log-user-out/" + id, Security.requestOptions(""))
              .then((response) => response.json())
              .then((data) => {
                if (data.error) {
